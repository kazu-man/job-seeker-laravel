      ...
      <div class="card mt-4">
        ...
        <div class="card-body">
          <form @submit.prevent="ResetPassword"> 
            ...
            <div class="form-group">
              <input type="submit" value="Register" class="btn btn-success w-100" style="margin:auto;margin-top:10px;">
            </div>
          </form>
        </div>
      </div>
    ...
    methods: {
        async ResetPassword(){ // パスワードリセットリクエストを投げる関数
            ...
            .then(res => {
                if(res.status == 200){
                console.log(res);
                console.log("password reset des");
                    this.$store.dispatch('common/alertModalUp', {data:200, successMessage:'パスワードを設定しました'});
        ...
        async checkToken(){
            await axios.get('/api/checkUrlBeforeChangePassword/' + this.form.token)
                .catch(() => {
